...
- Invalidation goes through `App\Support\ProductCache` and the nine observers registered by `AppServiceProvider::invalidateCatalogCache()`. All twelve files (the nine observers, `ProductCache` and the two `Observers/Concerns/` traits) are **mirrored byte-for-byte with `shop/`** — change both copies in the same commit, like the enums. `shop/tests/Feature/ProductCacheMirrorTest.php` fails the build on drift; add any new mirrored file to its `MIRRORED_CACHE_FILES` list.
- **Three shapes of catalog observer, and the difference decides which you write.** `Product`/`Variety`/`Image`/`Review` know their product, so they *forget* one page (`ForgetsProductCache`) — they fire on every sale, so they must stay cheap. `Category`/`Brand`/`Attribute`/`AttributeGroup` cannot: a renamed attribute reaches products through three pivots at once and a renamed category reaches its whole descendant subtree via breadcrumbs, so they *flush* both generations (`FlushesCatalogCache`), which is affordable only because such edits are rare. `Tag` is a third shape: it flushes only the **lists** (no product page renders a tag), hooks `saved` so a create counts (a new featured tag belongs on the home page at once), and flushes on an empty change set (`TagResource` syncs `attribute_tag` after the record saves clean) — so it deliberately inverts the rule below.
- **A metadata observer hooks `updated`, never `saved`** — the opposite of the per-product ones. `saved` also fires on insert with an empty `getChanges()`, so a conservative "unknown change → flush" rule would flush the catalog once per row of every seeder run. And `wasRecentlyCreated` is not a usable guard: it stays true for the instance's whole lifetime, so a create-then-edit in one request would wrongly skip the flush.
- **A metadata `delete` flushes unconditionally.** Deleting metadata rewrites product rows in the database with no Eloquent event firing on any product (`products.brand_id`, `products.attribute_group_id`, `varieties.attribute_id` are `nullOnDelete`; `product_attribute`/`attribute_variety` cascade), so nothing else would notice.
