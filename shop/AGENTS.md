...
- **Enums are mirrored from admin, not shared.** Every enum in `app/Enums` has a twin in `admin/app/Enums` with the
  **same case names and backing values** (labels differ: admin uses its own `lang` keys, the shop uses `enums.*`).
  The values are what the two apps agree on through the database, so changing one side alone silently breaks the
  other — staff save a value the storefront never recognises, with no error anywhere.
  `tests/Feature/EnumMirrorTest.php` fails the build on any mismatch, and on a new storefront enum that has no admin
  counterpart. When you add or change a case, do it in both apps in the same change.
