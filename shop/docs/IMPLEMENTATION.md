...
- **Catalog cache invalidation** (products + varieties, `CACHE.md` keys 5–7): the storefront caches product pages and category listings in Redis, and **this panel is what clears them**. Nine observers registered by `AppServiceProvider::invalidateCatalogCache()` call the mirrored `App\Support\ProductCache`, in three shapes: `Product`/`Variety`/`Image`/`Review` *forget* one product page precisely (they fire on every sale, so they must stay cheap), while `Category`/`Brand`/`Attribute`/`AttributeGroup` *flush* both generations — a product page renders all four live, and the affected product set is unbounded (a renamed attribute reaches products through three pivots; a renamed category reaches its whole descendant subtree via breadcrumbs); and `Tag` flushes only the *lists*, since no product page renders a tag. Four things to know before touching it: (1) nothing in this panel ever *reads* those entries, so a broken hook is invisible here and shows up only as a customer seeing a price or category name that was edited away — `ProductCacheInvalidationTest` is the only safety net; (2) it works solely because both apps share one Redis store with pinned `CACHE_PREFIX`/`REDIS_PREFIX` (they were silently mismatched before, each derived from `APP_NAME` with a different separator, while production was already on redis); (3) all twelve cache files are **mirrored byte-for-byte with `shop/`** — change both copies in one commit, as with the enums, and add any new one to `ProductCacheMirrorTest`'s `MIRRORED_CACHE_FILES`; (4) the metadata observers hook `updated` only, never `saved`, since `saved` also fires on insert with an empty change set and would flush the catalog once per row of every seeder run (`wasRecentlyCreated` is not a usable guard — it stays true for the instance's whole lifetime). Seeders that `truncate()` call `ProductCache::flushAll()`, since truncate fires no model events.
