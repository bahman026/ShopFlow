...
props.filters.attributeGroups.forEach((group) => {
    open['group-' + group.id] = false;
});

// Re-sync local state whenever a navigation swaps in fresh applied filters.
watch(
    () => props.applied,
    (applied) => {
        form.brands = [...applied.brands];
        form.attributes = [...applied.attributes];
...
const filteredBrands = computed(() => {
    ...
    return props.filters.brands.filter((brand) => brand.heading.includes(q));
});

const hasBrands = () => props.filters.brands.length > 0;
const hasAttributeGroups = () => props.filters.attributeGroups.length > 0;
...
<template>
    <aside class="flex flex-col rounded-xl border border-gray-100 bg-white">
        ...
        <!-- Brand -->
        <section v-if="hasBrands()" class="border-b border-gray-100 px-4 py-3">
            <button
                type="button"
                class="flex w-full items-center justify-between"
                @click="open.brand = !open.brand"
            >
                <span class="text-xs font-bold text-gray-700">برند</span>
                <Icon
                    :icon="faChevronDown"
                    class="text-[10px] text-gray-400 transition"
            ...
            <div v-show="open.brand" class="mt-3 flex flex-col gap-3">
                <input
                    v-model="brandQuery"
                    type="text"
                ...
                <div class="flex max-h-56 flex-col gap-2.5 overflow-y-auto">
                    ...
                    >
                        ...
                        <span class="text-xs text-gray-400">{{ formatNumber(brand.count) }}</span>
                    </label>
                    <p v-if="filteredBrands.length === 0" class="text-xs text-gray-400">
                        برندی یافت نشد.
                    </p>
                </div>
            </div>
        ...
        <!-- Attribute groups -->
        <section
            v-for="group in filters.attributeGroups"
            :key="group.id"
        ...
        >
            <button
                type="button"
                class="flex w-full items-center justify-between"
                @click="open['group-' + group.id] = !open['group-' + group.id]"
            >
                <span class="text-xs font-bold text-gray-700">{{ group.name }}</span>
                <Icon
                    :icon="faChevronDown"
                    class="text-[10px] text-gray-400 transition"
                    :class="{ 'rotate-180': open['group-' + group.id] }"
                />
            </button>

            <div
                v-show="open['group-' + group.id]"
                class="mt-3 flex max-h-56 flex-col gap-2.5 overflow-y-auto"
            >
                <label
                ...
                >
                    ...
                    <span class="text-xs text-gray-400">{{ formatNumber(attribute.count) }}</span>
                </label>
            </div>
        </section>

        <p
            v-if="!hasBrands() && !hasAttributeGroups() && !hasPriceRange()"
            class="px-4 py-3 text-xs text-gray-400"
        >
            فیلتر دیگری برای این دسته تعریف نشده است.
        </p>
...
.price-slider input[type='range']::-webkit-slider-thumb {
    ...
    background: #fff;
    border: 2px solid var(--color-brand, #e1002a);
    cursor: pointer;
