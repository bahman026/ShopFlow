...
return [
    ...
    */
    ...
    'include_files' => [
        // Two folders in the UI, one per app, so admin and storefront logs are
        // never mixed together in one list.
        //
        // The storefront runs in its own container, so in production its files
        // arrive on a volume both containers mount (LOG_VIEWER_SHOP_PATH). The
        // dev default reaches straight into the sibling app, which works only
        // because the whole repo is bind-mounted there.
        env('LOG_VIEWER_ADMIN_PATH', storage_path('logs')) . '/*.log' => 'Admin panel',
        env('LOG_VIEWER_SHOP_PATH', base_path('../shop/storage/logs')) . '/*.log' => 'Storefront',
    ],
