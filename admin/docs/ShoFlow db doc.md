...
* `latitude` / `longitude` are nullable columns (defined in `create_addresses_table`); the storefront fills them from a Neshan map when the customer adds an address.
